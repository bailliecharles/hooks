btShapes <- function(scores, vectors, fcn, pcs = 1:2, n = c(3,4), m = 3, row.names = NULL, 
	...
	n_shapes2= c(26,27,28,29,30,21,22,23,24,25,16,17,18,19,20,11,12,13,14,15,6,7,8,9,10,1,2,3,4,5)
	# Plot each shape
	for(i in n_shapes2) {
		#for(j in 1:n_shapes2){
		...
		#do.call(what=fcn, list(xy=scores_bt[j,pcs], coor=lm_arr[, , j], ...))
